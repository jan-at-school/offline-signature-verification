...
def binarize(img, thre, width, height):
    for x in range(0, width):
        for y in range(0, height):
            ...
            # get the pixel and average all of them
            avg = (r + g + b) / 3
...
def centroid(img, x1, y1, x2, y2):
    ...
    for y in range(y1, y2):
        for x in range(x1, x2):
            r, g, b = img.getpixel((x, y))
            # For black pixel. update the current storage
            avg = (r + g + b) / 3
            if avg == 0:
                cx += x
                cy += y
...
def blackToWhiteTransition(img, x1, y1, x2, y2):
    ...
    for y in range(y1, y2):
        for x in range(x1, x2):
            # Current Pixel is Black and Next is white, add up
            r, g, b = img.getpixel((x, y))
            avg = (r + g + b) / 3
            if(x + 1 == width):
                continue
...
def spliceImage(img, x1, y1, x2, y2, recVal, spliceVal=""):
    ...
    if recVal < 3:
        ...
        summationAngleList[spliceVal] = blackPixelAngleSummation(
            img, x1, y1, x2, y2)
